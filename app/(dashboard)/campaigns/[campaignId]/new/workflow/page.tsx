...
const CAMPAIGN_STEPS = [
  { id: 'workflow', name: 'Workflow', description: 'Build your workflow', icon: BarChart3, color: 'text-orange-500', status: 'current' as const },
  { id: 'audience', name: 'Audience', description: 'Target settings', icon: Users, color: 'text-blue-500', status: 'upcoming' as const },
  { id: 'identity', name: 'Identity', description: 'Sender profile', icon: UserCheck, color: 'text-green-500', status: 'upcoming' as const },
...
export default function WorkflowPage() {
  ...
  const [campaignSteps] = useState(CAMPAIGN_STEPS);

  // Active step state for accordion functionality
  ...
  const handleCloseAudiencePanel = useCallback(() => {
    ...
    setAudiencePanelView('options'); // Reset to options view when closing
  }, []);

  // CRM Import handlers
  const handleCrmImportClick = useCallback(() => {
  ...
  const handleLinkedInImportClose = useCallback(() => {
    setShowLinkedInImport(false);
    setLinkedInImportStep(1);
  }, []);

  const handleLinkedInImportNext = useCallback(() => {
    setLinkedInImportStep(prev => prev + 1);
  }, []);

  const handleLinkedInImportBack = useCallback(() => {
    setLinkedInImportStep(prev => Math.max(1, prev - 1));
  }, []);

  // Import People screen handlers
  ...
  // Import view navigation handlers
  const handleImportOptionClick = useCallback((viewName: typeof activeImportView) => {
    setActiveImportView(viewName);
  }, []);
                        ...
                        {isActive && (
                          ...
                          >
                            <div className="px-4 pb-4">
                              ...
                              {step.id === 'workflow' && <WorkflowContent />}
                              {step.id === 'audience' && <AudienceContent />}
                              {step.id === 'identity' && <IdentityContent />}
                              {step.id === 'content' && <ContentContent />}
                              {step.id === 'launch' && <LaunchContent />}
                              {step.id === 'settings' && <SettingsContent />}
            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
